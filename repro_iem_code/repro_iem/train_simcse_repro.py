...
from repro_iem.runtime import CustomArguments, DataArguments, ModelArguments, load_train_dataset
...
class SimCSETrainer(Trainer):
    ...
    def compute_loss(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], return_outputs: bool = False):
...
def main():
    ...
    dataset = load_train_dataset(data_args)
    if data_args.max_train_samples is not None:
        limit = min(len(dataset), data_args.max_train_samples)
        train_examples = [dataset[index] for index in range(limit)]
    else:
        train_examples = [dataset[index] for index in tqdm(range(len(dataset)), desc="Loading train examples...", disable=not accelerator.is_main_process)]
